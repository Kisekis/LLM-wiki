  ...
  */
  ...
  visit(fileTree, node => {
    ...
    if (node.type === "text") {
      if (
        errFlag.WIKINotBold &&
        node.value.search(/LLM\sSecurity\sWiki/) !== -1 &&
        node.value.search(/\*\*LLM\sSecurity\sWiki\*\*/) === -1
      ) {
        errList.push(WIKINotBold(file, node.position?.start.line, node.position?.start.column));

        if (parg.r) errList.push("[Info] Suggestion: '**LLM Security Wiki**'");
      }
...
function WIKINotBold(file: string, line: number | undefined, column: number | undefined) {
  return "[Warning] [WIKINotBold] 'LLM Security Wiki' is not bold in file " + file + ":" + line + ":" + column;
}
